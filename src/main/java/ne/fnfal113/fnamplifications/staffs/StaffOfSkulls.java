...
public class StaffOfSkulls extends AbstractStaff {
    ...
    public void onClick(PlayerInteractEvent event) {
        ...
        for(int i = 0; i < 7; i++) {
            WitherSkull skull = (WitherSkull) player.getWorld().spawnEntity(player.getEyeLocation().clone().add(player.getLocation().getDirection().clone().multiply(i).setY(-0.5)), EntityType.WITHER_SKULL);
