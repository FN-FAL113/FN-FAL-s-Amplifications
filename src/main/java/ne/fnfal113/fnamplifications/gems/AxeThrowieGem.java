...
public class AxeThrowieGem extends AbstractGem implements OnRightClickHandler {
    ...
    public void onDrag(Player player, SlimefunItem slimefunGemItem, ItemStack gemItem, ItemStack itemStackToSocket) {
        if (WeaponArmorEnum.AXES.isTagged(itemStackToSocket.getType())) {
            ...
            Utils.sendMessage("Invalid item to socket! Gem works on axes only", player);
        }
    }
    ...
    public void onRightClick(Player player) {
        ...
        if(!hasPermissionToThrow(player)) {
            Utils.sendMessage("You don't have the permission to throw here! (Needs block interaction flag enabled)", player);
